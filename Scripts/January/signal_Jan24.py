...
def ppE_to_h(b,beta,sp,sc):
	vf_sp = np.zeros((len(sp.sample_frequencies)))
	vf_sc = np.zeros((len(sc.sample_frequencies)))
	ppE_factor_sp = np.zeros((len(sp.sample_frequencies)),dtype=np.complex_)
	ppE_factor_sc = np.zeros((len(sc.sample_frequencies)),dtype=np.complex_)
	hp = np.zeros((len(sp.sample_frequencies)),dtype=np.complex_)
	hc = np.zeros((len(sc.sample_frequencies)),dtype=np.complex_)

	for i in range(1,len(sp.sample_frequencies)):
		vf_sp[i] = (np.pi*m*sp.sample_frequencies[i])**(1.0/3.0)    
                vf_sc[i] = (np.pi*m*sc.sample_frequencies[i])**(1.0/3.0)
		ppE_factor_sp[i] = e**(beta * vf_sp[i]**b * 1j)
		ppE_factor_sc[i] = e**(beta * vf_sc[i]**b * 1j)
		hp[i] = sp[i] * ppE_factor_sp[i]
		hc[i] = sc[i] * ppE_factor_sc[i]
